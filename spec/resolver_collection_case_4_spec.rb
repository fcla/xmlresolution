...
describe ResolverCollection do
  ...
  def xmlcases_instance_doc
    File.join(@@files, 'internal-dtd.xml')
  end
 ...
 it "should be a failure  eventOutcome " do
       premis_report =  @@xmlcases_resolver.premis_report
       premis_report.index('<eventOutcome>failure</eventOutcome>').should_not be_nil
  end
