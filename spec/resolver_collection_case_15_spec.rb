...
###	warn_level = $VERBOSE
###	  $VERBOSE = nil
  XmlResolution.class_variable_set(:@@store, nil);
  files = File.join(File.dirname(__FILE__), 'files', 'example-xml-documents')
  XmlResolution.class_variable_set(:@@files,files)
  def proxy 
  ...
  def what_should_get_resolved
    ['http://schema.fcla.edu/xml/rss-0.91.dtd']
  end
  ...
  def xmlcases_instance_doc
    File.join(XmlResolution.class_variable_get(:@@files), 'rss-system.xml')
  end
  ...
  before(:all) do  
     XmlResolution.class_variable_set(:@@store, Dir.mktmpdir('resolver-store-', '/tmp'))
     FileUtils.mkdir_p File.join(XmlResolution.class_variable_get(:@@store), 'schemas')
     FileUtils.mkdir_p File.join(XmlResolution.class_variable_get(:@@store), 'collections')
  end 

  after(:all) do
    FileUtils::rm_rf XmlResolution.class_variable_get(:@@store)
  end

  it "should create new collections" do
    ResolverCollection.new(XmlResolution.class_variable_get(:@@store), collection_name_1)

    ResolverCollection.collections(XmlResolution.class_variable_get(:@@store)).include?(collection_name_1).should == true
  end

  it "should save document resolution data" do

    # resolve three representative documents:
    XmlResolution.class_variable_set(:@@xmlcases_resolver, XmlResolver.new(File.read(xmlcases_instance_doc),   file_url(xmlcases_instance_doc),   XmlResolution.class_variable_get(:@@store), proxy))

    # save the data collected to our collection

    XmlResolution.class_variable_get(:@@xmlcases_resolver).save(collection_name_1)

    # Let's get the collection:

    collection = ResolverCollection.new(XmlResolution.class_variable_get(:@@store), collection_name_1)
    ...
    doc_ids.include?(XmlResolution.class_variable_get(:@@xmlcases_resolver).document_identifier).should   == true
  end


    it "should be a success eventOutcome " do
	    premis_report =  XmlResolution.class_variable_get(:@@xmlcases_resolver).premis_report
  ...
  it "should give the collapsed list of schemas" do
    ...
    #@@redirects = schema_redirects.keys.sort { |a,b| a.downcase <=> b.downcase }
    XmlResolution.class_variable_set(:@@redirects, redirects)
    XmlResolution.class_variable_get(:@@redirects).count.should == 0  # 
    ...
    #@@locations = schema_locs.keys.sort { |a,b| a.downcase <=> b.downcase }
    XmlResolution.class_variable_set(:@@locations,locations)
    XmlResolution.class_variable_get(:@@locations).count.should == 1  
    
    # Make sure all the successfully downloaded schemas in the resolution objects are listed somewhere in the manifest:

    XmlResolution.class_variable_set(:@@manifest,collection.manifest)

    XmlResolution.class_variable_get(:@@locations).each { |loc|  /#{loc}/.should =~ XmlResolution.class_variable_get(:@@manifest ) }
    ...
    XmlResolution.class_variable_get(:@@locations).each do |loc| 
  ...
  it "should resolve these" do
    what_should_get_resolved.each{|z| XmlResolution.class_variable_get(:@@locations).include?(z).should be_true}
  end
